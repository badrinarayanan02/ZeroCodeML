...
if data is not None:
    ...
    if target_values != '':
        ...
        models = pd.DataFrame({
            'Model': ['Logistic Regression', 'Linear SVC', 'K-Nearest Neighbor', 'Multinomial Naive Bayes','Decision Tree', 'Random Forest','Gradient Boosting', 'XGBoost'],
            'Recall': [recall_lr, recall_svm, recall_knn, recall_mnb, recall_dt, recall_rf, recall_gbc, recall_xgboost],
            'Accuracy': [acc_lr, acc_svm, acc_knn, acc_mnb, acc_dt, acc_rf, acc_gbc, acc_xgboost],
            'Precission': [prec_lr, prec_svm, prec_knn, prec_mnb, prec_dt, prec_rf, prec_gbc, prec_xgboost],
            'F1 Score': [f1_lr, f1_svm, f1_knn, f1_mnb, f1_dt, f1_rf, f1_gbc, f1_xgboost],
        })
        st.write('Done!')
